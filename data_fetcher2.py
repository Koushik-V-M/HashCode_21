...
logging.basicConfig(filename='data-fetcher2.log', level=logging.ERROR,
                    format='%(asctime)s %(levelname)s %(name)s %(message)s')
logger = logging.getLogger(__name__)

df = pd.read_csv("./Data/all_images2.csv", index_col=None)
consensus_df = pd.read_csv("./Data/consensus_data2.csv", index_col=None)
...
for index, row in df.loc[247800:].iterrows():
    ...
    try:
        ...
        f = open("./Data/images2/"+str(row["CaptureEventID"])+".JPG", 'wb')
        f.write(r.content)
        f.close()

        im = Image.open("./Data/images2/"+str(row["CaptureEventID"])+".JPG")
        im = im.resize((224, 224))

        os.remove("./Data/images2/"+str(row["CaptureEventID"])+".JPG")
        im = im.save("./Data/images2/"+str(row["CaptureEventID"])+".JPG")
